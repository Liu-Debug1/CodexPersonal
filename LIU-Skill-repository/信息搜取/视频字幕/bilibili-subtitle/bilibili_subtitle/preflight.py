...
def _status_symbols() -> dict[CheckStatus, str]:
    unicode_symbols = {
        CheckStatus.OK: "✅",
    ...
    if all(_can_encode(symbol) for symbol in unicode_symbols.values()):
...
def check_bbdown() -> CheckResult:
    ...
    try:
        ...
        if result.returncode != 0 and "required argument missing" in version.lower():
            version = "installed"
    except Exception:
        version = "installed"
...
def main() -> int:
    ...
    if args.json:
        ...
        report.print_report()
        print()
        ok, error = _status_symbols()[CheckStatus.OK], _status_symbols()[CheckStatus.ERROR]
        print(f"{ok} Ready to proceed" if report.can_proceed else f"{error} Fix errors before proceeding")
