...
if ([string]::IsNullOrWhiteSpace($RepositoryRoot) -and $null -ne $config) { $RepositoryRoot = [string]$config.repositoryRoot }
if ([string]::IsNullOrWhiteSpace($PackagesDirectoryName) -and $null -ne $config) { $PackagesDirectoryName = [string]$config.packagesDirectoryName }
if ([string]::IsNullOrWhiteSpace($RuntimeDirectoryName) -and $null -ne $config) { $RuntimeDirectoryName = [string]$config.runtimeDirectoryName }
if ([string]::IsNullOrWhiteSpace($TargetAgent)) {
    $configuredTargetAgent = if ($null -ne $config) { $config.PSObject.Properties['targetAgent'] } else { $null }
    $TargetAgent = if ($null -ne $configuredTargetAgent -and -not [string]::IsNullOrWhiteSpace([string]$configuredTargetAgent.Value)) { [string]$configuredTargetAgent.Value } else { 'Codex' }
}
foreach ($requiredValue in @($RepositoryRoot, $PackagesDirectoryName, $RuntimeDirectoryName, $TargetAgent)) {
    if ([string]::IsNullOrWhiteSpace([string]$requiredValue)) {
        throw 'repositoryRoot, packagesDirectoryName, runtimeDirectoryName, and targetAgent are required.'
    }
}
...
function Get-ManagedChildPath {
    ...
    if ([string]::IsNullOrWhiteSpace($SkillName) -or $SkillName.StartsWith('.')) {
        throw "Managed Skill name is invalid: $SkillName"
    }

    $targetRoot = [System.IO.Path]::GetFullPath($TargetSkillsPath).TrimEnd('\')
    $childPath = [System.IO.Path]::GetFullPath((Join-Path $targetRoot $SkillName))
    if (-not (Split-Path -Parent $childPath).Equals($targetRoot, [System.StringComparison]::OrdinalIgnoreCase)) {
        throw "Managed Skill path must be a direct child of the target skills directory: $SkillName"
    }

    return $childPath
}

function Get-ManagedTargetSnapshot {
    param(
        [Parameter(Mandatory = $true)][string]$TargetSkillsPath,
        [Parameter(Mandatory = $true)]$SkillEntries
    )

    if (-not (Test-Path -LiteralPath $TargetSkillsPath -PathType Container)) {
        return $null
    }

    Test-NoReparsePoint -Path $TargetSkillsPath
    $entries = foreach ($skill in @($SkillEntries)) {
        $targetSkillPath = Get-ManagedChildPath -TargetSkillsPath $TargetSkillsPath -SkillName $skill.Name
        if (-not (Test-Path -LiteralPath $targetSkillPath -PathType Container)) {
            continue
        }

        Test-NoReparsePoint -Path $targetSkillPath
        foreach ($file in @(Get-ChildItem -LiteralPath $targetSkillPath -File -Force -Recurse | Sort-Object FullName)) {
            $relativePath = Get-RelativePath -BasePath $TargetSkillsPath -Path $file.FullName
            $fileHash = (Get-FileHash -LiteralPath $file.FullName -Algorithm SHA256).Hash.ToLowerInvariant()
            "$relativePath`t$fileHash"
        }
    }
...
function Get-DeploymentScope {
    ...
    $scopeProperty = $Manifest.PSObject.Properties['deploymentScope']
    $scope = if ($null -eq $scopeProperty -or [string]::IsNullOrWhiteSpace([string]$scopeProperty.Value)) { 'replace-target-root' } else { [string]$scopeProperty.Value }
    if ($scope -notin @('replace-target-root', 'managed-subdirectories')) {
        throw "Unsupported deploymentScope: $scope"
    }
...
function New-Context {
    ...
    $codexRoot = $script:RepositoryRoot
    $packagesRoot = Join-Path $codexRoot $script:PackagesDirectoryName
    ...
    if ($manifest.targetAgent -ne $script:TargetAgent) {
        throw "Manifest targetAgent does not match the configured target Agent. Expected: $($script:TargetAgent); actual: $($manifest.targetAgent)"
    }
    ...
    $deploymentScope = Get-DeploymentScope -Manifest $manifest
    $targetSkillsPathProperty = $manifest.PSObject.Properties['targetSkillsPath']
    if ($null -ne $targetSkillsPathProperty -and -not [string]::IsNullOrWhiteSpace([string]$targetSkillsPathProperty.Value)) {
        $targetSkillsPath = [System.IO.Path]::GetFullPath([string]$targetSkillsPathProperty.Value)
        if (-not (Test-Path -LiteralPath $targetSkillsPath -PathType Container)) {
            throw "Declared targetSkillsPath does not exist: $targetSkillsPath"
        }
        $projectCodexPath = Split-Path -Parent $targetSkillsPath
    }
    else {
        $projectCodexPath = Join-Path $projectPath $script:RuntimeDirectoryName
...
function Test-Preflight {
    ...
    $targetSnapshot = if ($Context.DeploymentScope -eq 'managed-subdirectories') {
        Get-ManagedTargetSnapshot -TargetSkillsPath $Context.TargetSkillsPath -SkillEntries $skillEntries
    }
    else {
        Get-DirectorySnapshot -Path $Context.TargetSkillsPath
    }
...
function Deploy-ManagedSubdirectories {
    param(
        [Parameter(Mandatory = $true)]$Context,
        [Parameter(Mandatory = $true)]$Preflight
    )

    $desiredNames = @($Preflight.SkillEntries | ForEach-Object { $_.Name })
    $previousNames = @($Context.Manifest.managedAssets.skills | ForEach-Object {
        if ($_ -is [string]) { [string]$_ } elseif ($null -ne $_.name) { [string]$_.name } else { $null }
    } | Where-Object { -not [string]::IsNullOrWhiteSpace($_) })
    $obsoleteNames = @($previousNames | Where-Object { $_ -notin $desiredNames })

    foreach ($skillName in $obsoleteNames) {
        $targetSkillPath = Get-ManagedChildPath -TargetSkillsPath $Context.TargetSkillsPath -SkillName $skillName
    ...
    foreach ($skill in @($Preflight.SkillEntries)) {
        $sourceSkillPath = Join-Path $Context.PackageSkillsPath $skill.Name
        $targetSkillPath = Get-ManagedChildPath -TargetSkillsPath $Context.TargetSkillsPath -SkillName $skill.Name
        if (Test-Path -LiteralPath $targetSkillPath) {
            Test-NoReparsePoint -Path $targetSkillPath
            Remove-Item -LiteralPath $targetSkillPath -Recurse -Force
        }
        Copy-Item -LiteralPath $sourceSkillPath -Destination $Context.TargetSkillsPath -Recurse -Force
    }
}
...
function Remove-DeclaredLegacyPaths {
    ...
    $legacyProperty = $Context.Manifest.PSObject.Properties['legacyPathsToRemove']
    $legacyPaths = if ($null -eq $legacyProperty) { @() } else { @($legacyProperty.Value) }
...
function Update-ManifestAndDocumentation {
    ...
    $existingSkillMap = @{}
    foreach ($existingSkill in @($Context.Manifest.managedAssets.skills)) {
        $existingName = if ($existingSkill -is [string]) { [string]$existingSkill } elseif ($null -ne $existingSkill.name) { [string]$existingSkill.name } else { $null }
        if (-not [string]::IsNullOrWhiteSpace($existingName)) {
            $existingSkillMap[$existingName] = $existingSkill
        }
    }
    ...
    $manifestSkills = foreach ($skill in $Preflight.SkillEntries) {
        ...
        $existingSkill = if ($existingSkillMap.ContainsKey($skill.Name)) { $existingSkillMap[$skill.Name] } else { $null }
        if ($null -ne $existingSkill) {
            foreach ($property in @($existingSkill.PSObject.Properties | Where-Object { $_.Name -notin @('name', 'source', 'version', 'sha256') })) {
                if (-not $record.Contains($property.Name)) {
    ...
    if ($null -eq $Context.Manifest.PSObject.Properties['skillsLastUpdated']) {
        $Context.Manifest | Add-Member -MemberType NoteProperty -Name skillsLastUpdated -Value $null
    }
    if ($null -eq $Context.Manifest.PSObject.Properties['lastVerifiedAt']) {
        $Context.Manifest | Add-Member -MemberType NoteProperty -Name lastVerifiedAt -Value $null
    }
    if ($null -eq $Context.Manifest.PSObject.Properties['contentSha256']) {
        $Context.Manifest | Add-Member -MemberType NoteProperty -Name contentSha256 -Value $null
    }
    if ($null -eq $Context.Manifest.PSObject.Properties['fileCount']) {
        $Context.Manifest | Add-Member -MemberType NoteProperty -Name fileCount -Value $null
    }
    ...
    $fixtureProperty = $Context.Manifest.PSObject.Properties['isTestFixture']
...
function Show-Plan {
    ...
    $manifestContentHash = if ($null -eq $Context.Manifest.PSObject.Properties['contentSha256']) { $null } else { [string]$Context.Manifest.contentSha256 }
    if ([string]::IsNullOrWhiteSpace($manifestContentHash)) {
        Write-Output 'Manifest status: no prior Skill content hash is recorded.'
    }
    elseif ($manifestContentHash -eq $Preflight.PackageSnapshot.ContentSha256) {
    ...
    if ($null -eq $Preflight.TargetSnapshot) {
        ...
        if ($Context.DeploymentScope -eq 'managed-subdirectories') {
            Write-Output 'Target status: only manifest-managed direct Skill subdirectories will be replaced; other directories are preserved.'
        }
        else {
...
switch ($Mode) {
    ...
    'DeploySkills' {
        Show-Plan -Context $context -Preflight $preflight
        if ($context.DeploymentScope -eq 'managed-subdirectories') {
            Deploy-ManagedSubdirectories -Context $context -Preflight $preflight
            $afterDeployment = Get-ManagedTargetSnapshot -TargetSkillsPath $context.TargetSkillsPath -SkillEntries $preflight.SkillEntries
        }
        else {
            if (Test-Path -LiteralPath $context.TargetSkillsPath) {
                Remove-Item -LiteralPath $context.TargetSkillsPath -Recurse -Force
            }
            if (-not (Test-Path -LiteralPath $context.PackageSkillsPath -PathType Container)) {
                New-Item -ItemType Directory -Path $context.PackageSkillsPath -Force | Out-Null
            }
